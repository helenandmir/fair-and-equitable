...
class Hungarian:
    ...
    def optimal_assignment(self,matrix):
        while True:
            # Step 1: Check if a perfect matching exists using the Hungarian algorithm
            row_ind, col_ind = scipy.optimize.linear_sum_assignment(matrix)
            if len(row_ind) == matrix.shape[0]:
                # Perfect matching found, return the row and column indices
                return row_ind, col_ind

            # Step 2: Find the smallest value in the matrix that is not zero and subtract it from all non-zero elements
            non_zero_elements = matrix[matrix > 0]
            if non_zero_elements.size == 0:
                break
            min_value = np.min(non_zero_elements)
            matrix[matrix > 0] -= min_value
    def play_hungarian_algo(self):
        self.Matrix = np.array(self.Matrix)
        row_ind, col_ind = self.optimal_assignment(self.Matrix)
        # Create a list of colors repeated according to the counts in dictionary A
        colors = []
        for color, count in self.colors_required.items():
...
if __name__ == '__main__':
    ...
    print(before_list)
    print("Min:")
    print("The MaxSum before running Hungarian algo = {}".format(np.min(before_list)))
    print("The MaxSum after running Hungarian algo = {}".format(np.min(list_rep)))
