...
def dis(p1,p2):
    fileA = "LongTimeData/diabetes.csv"
    df1 = pd.read_csv(fileA)
    all_point = list(df1["ID"])
    dic_X = dict(zip(df1["ID"], df1["X"]))
    dic_Y = dict(zip(df1["ID"], df1["Y"]))
    dic_Z = dict(zip(df1["ID"], df1["Z"]))

    return math.sqrt((dic_X[p2] - dic_X[p1]) ** 2 + (dic_Y[p2]- dic_Y[p1]) ** 2 + (dic_Z[p2]- dic_Z[p1]) ** 2)
